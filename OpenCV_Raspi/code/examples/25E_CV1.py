...
lower_laser = np.array([80, 0, 182])   # 红色下限 (H,S,V)
upper_laser = np.array([179, 255, 255]) # 红色上限
...
while cap.isOpened():
    ...
    if max_rect is not None:
        ...
        print([max_rect.astype(int)])
    else:
        print("no rectangle detected")

 
    
    # print(f"laser blob: ({x_laser}, {y_laser})")
...
def Kalman_filter_cv2(x_obs,y_obs):
    ...
    # 记录滤波后的状态
    x_filtelaser.append(kf.x[0])
    y_filtelaser.append(kf.x[1])
    return kf.x[0],kf.x[1]

def Figure()  :
    plt.figure(figsize=(12, 6))
    plt.plot(x_observed,y_observed, 'r.', label='Noisy Observations')
    plt.plot(x_filtelaser, y_filtelaser, 'b-', linewidth=2, label='Kalman Filtelaser')
    plt.legend()
