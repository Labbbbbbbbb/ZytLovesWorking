...
void usart_SendByte(uint16_t data)
{
	...
	while(!(MOTOR_USART->SR & UART_FLAG_TXE))
	{
...
// {
//   if(huart->Instance == MOTOR_USART) // 当前串口1
//   {
//     fifo_enQueue((uint8_t)(huart->Instance->DR & (uint16_t)0x01FF));
//     rxCount = fifo_queueLength(); for(int i=0; i < rxCount; i++) { rxCmd[i] = fifo_deQueue(); }
//     HAL_UART_Receive_IT(&huart1, (uint8_t *)&(huart->Instance->DR), 1); // 重新开启中断
//   }
// }
/**
	* @brief   USART1中断函数
